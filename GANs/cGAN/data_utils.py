# For part of bellowed code please refer to https://github.com/tdeboissiere/DeepLearningImplementations/tree/master/pix2pix/
...
def load_data(dset, image_data_format):
    ...
    with h5py.File('<LINK TO DATA>', "r") as hf:
...
def gen_batch(X1, X2, batch_size):
    ...
    while True:
        idx = np.random.choice(X1.shape[0], batch_size, replace=True)
        yield X1[idx], X2[idx]
...
def plot_generated_batch(X_full, X_sketch, generator_model, batch_size, image_data_format, suffix, logging_dir):
    ...
    plt.axis("off")
    # TBD: Proper saving
    plt.savefig(os.path.join(logging_dir, "<PATH>" % random.randint(1,10000)))
    plt.clf()
    plt.close()
